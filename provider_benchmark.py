...
def parseArgs():
    ...
    parser.add_argument('-v', '--verbose', action='store_true', help="display progess of tests")
    ...
    connection.add_argument('-websocket', action='store_true', help="test websocket")
    connection.add_argument('-http', action='store_true', help="test http")
    test = parser.add_mutually_exclusive_group(required=True)
    test.add_argument('-1', '--log_consistency', action='store_true', help="run test for log consistency")
    test.add_argument('-2', '--pending_transactions', action='store_true', help="run test for pending transactions")
...
def alchemy(tests, args):
    if args.websocket:
        if args.mainnet:
            web3 = Web3(Web3.WebsocketProvider("wss://eth-mainnet.ws.alchemyapi.io/ws/" + args.alchemy_api_key))
        else:
            web3 = Web3(Web3.WebsocketProvider("wss://eth-ropsten.ws.alchemyapi.io/ws/" + args.alchemy_api_key))
    else: 
        if args.mainnet:
            web3 = Web3(Web3.HTTPProvider("https://eth-mainnet.alchemyapi.io/jsonrpc/" + args.alchemy_api_key))
        else:
            web3 =  Web3(Web3.HTTPProvider("https://eth-ropsten.alchemyapi.io/jsonrpc/" + args.alchemy_api_key))
    runTests(web3, "Alchemy", tests)

def infura(tests, args):
    if args.websocket:
        if args.mainnet:
            web3 = Web3(Web3.WebsocketProvider("wss://mainnet.infura.io/ws/v3/" + args.infura_api_key))
        else:
            web3 = Web3(Web3.WebsocketProvider("wss://ropsten.infura.io/ws/v3/" + args.infura_api_key))
    else: 
        if args.mainnet: 
            web3 = Web3(Web3.HTTPProvider("https://mainnet.infura.io/v3/" + args.infura_api_key))
        else:
            web3 = Web3(Web3.HTTPProvider("https://ropsten.infura.io/v3/" + args.infura_api_key))
