...
def transactionsTest(web3, company):
    ...
    sendAddress = input("Enter an address to send transaction through " +
                        company + " (must be distinct, do not repeat accross tests): ")
    sendPrivateKey = input("Enter the private key for that address: ")
    recieveAddress = input("Enter an address to recieve transaction through " +
                           company + " (must be distinct, do not repeat accross tests): ")
    ...
    for i in range(config.ITERATIONS):
        ...
        try:
            ...
            if config.VERBOSE:
                print(
                    company, "gave the following error when sending then getting a transaction:", e)
        ...
        if config.VERBOSE:
            print(company, "returned the following reciept:", receipt)

        time.sleep(config.DELAY)

    print(company, "failed to find pending transactions:",
